...
export default function Ecctrl({
	...
	const jumpLandAnimation = !animated ? null : useGame(state => state.jumpLand)
	...
	const moveCharacter = (
		...
		if (actualSlopeAngle < 1 && Math.abs(slopeAngle) > 0.2 && Math.abs(slopeAngle) < 1) {
	...
	useEffect(() => {
		...
		if (followLight) {
			dirLight = characterModelRef.current.parent.parent.children.find(
				item => item.type === 'DirectionalLight'
	...
	useFrame((state, delta) => {
		...
		characterModelRef.current.quaternion.rotateTowards(modelQuat, delta * turnSpeed)
